...
struct LoginTextFieldsView: View {
  ...
  @StateObject var model           = LoginTextFieldsManager()
...
extension LoginTextFieldsView {
  ...
  private var textFieldsAndErrorMessage: some View {
    ...
    //MARK: - USERNAME
    if withUsername {
      LoginTextField(text: $model.username, field: .username, isSecure: false) {
        // TODO: Validate
    ...
    //MARK: - EMAIL
    LoginTextField(text: $model.email, field: withUsername ? .newEmail : .email, isSecure: false) {
    ...
    //MARK: - PASSWORD
    LoginTextField(text: $model.password, field: withConfirmation ? .newPassword : .password, isSecure: true) {
      model.validate(withConfirmation: true)
    ...
    if withConfirmation {
      LoginTextField(text: $model.confirmPassword, field: .confirmPassword, isSecure: true) {
        model.validate(withConfirmation: true)
        focus = nil
      } xMarkAction: {
  ...
  private var buttonLabel: some View {
    Text(buttonText)
      .buttonLabel(background: Color.accentColor, foreground: .systemBackground)
  }
  
  @ViewBuilder //MARK: - PASSW RECOVERY ALERT
  private var alert: some View {
    LoginTextField(text: $recoveryEmail, field: .email, isSecure: false) { } xMarkAction: { }
...
struct AuthFieldsView_Previews: PreviewProvider {
  static var previews: some View {
    LoginTextFieldsView(model: LoginTextFieldsManager(), resetPassword: true, withConfirmation: true, buttonText: "Button Text", buttonColor: .blue, withUsername: false) { }
  }
}
